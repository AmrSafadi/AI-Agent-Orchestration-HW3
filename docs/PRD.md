...
| Meet engineering standards | Ruff 0 violations; test coverage >= 85% (currently 93.38%, gate 85%, across 101 passing tests and 2 skipped integration tests); files <= 150 code lines; `uv` only. |
...
(18 pages, 0 overfull boxes, 101 tests passing, 2 skipped, 93.38% coverage with an 85% gate).
Reproducing the PDF from scratch requires a free TeX toolchain (LuaLaTeX + biber)
