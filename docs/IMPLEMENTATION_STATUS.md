...
| LaTeX Renderer + Compiler (Phase E) | Mostly complete | `latex/escaping.py`, 5 Jinja templates, `latex/renderer.py` (renders `main.tex` with cover/TOC/figures/table/formula/BiDi/bibliography), `latex/compiler.py` (multi-pass, graceful). Pending: actual PDF compile (needs a TeX toolchain), per-chapter files, asset copy, xelatex fallback. |
