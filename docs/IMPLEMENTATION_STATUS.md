...
| 8. PDF Compilation | Compile final PDF with LuaLaTeX/XeLaTeX and bibliography backend. | Remaining — blocked only on installing a free TeX toolchain (lualatex+biber) + Hebrew font (David CLM). Capture build logs and validation report. |
...
- The actual compiled `final.pdf` — blocked on a free TeX toolchain (lualatex+biber) + Hebrew font (David CLM).
