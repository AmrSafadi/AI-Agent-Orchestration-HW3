...
| Language | The primary language and script direction of the document. | The document is primarily Hebrew (RTL) via `\setmainlanguage{hebrew}` / `\setmainfont{David CLM}`, with English kept inline only for technical terms; the `hebrew_english_section` is an explicit `\begin{english}` BiDi demo block, not the document's primary language. |

Quality and entry point: 145 tests pass, 2 skips, coverage 94.31% (gate 85%), ruff 0 violations. The CLI is `uv run --no-project --with pydantic --with matplotlib --with jinja2 python -m bookgen.main --dry-run [--build-pdf] [--run-crew]`. Running `--build-pdf` produces a 19-page Hebrew-primary football analytics `final.pdf`, and a snapshot copy is committed at the repository root so a grader sees it on clone.
...
The system uses LaTeX templates and a deterministic compiler wrapper. The LaTeX Agent only produces assembly intent; Python renders the `.tex` (`renderer.py`) and compiles it (`compiler.py`, running `lualatex` -> `biber` -> `lualatex` -> `lualatex`). Rendering and PDF compilation are implemented, exercised by tests, and verified end-to-end: the compiled 19-page Hebrew-primary football analytics `final.pdf` is produced and committed at the repository root, with the cover, table of contents, embedded image, Python-generated graph, table, mathematical formula, Hebrew-English BiDi block, and the bibliography (3 sources resolved) all rendering correctly. Reproducing the PDF from scratch requires a free TeX toolchain (`lualatex` + `biber`) plus the Hebrew `David CLM` font (culmus package); the default `--dry-run` path does not compile.
