...
Run the full suite (`python -m pytest tests`). The project currently has 101 passing tests, 2 skipped integration tests, and 93.38% coverage (gate 85%). To enforce the coverage gate, add `--cov=bookgen --cov-fail-under=85` (also configured via `fail_under=85` in `pyproject.toml`).
