...
Run the full suite (`python -m pytest tests`). The project currently has 89 tests at 92.27% coverage (gate 85%). To enforce the coverage gate, add `--cov=src/bookgen --cov-fail-under=85` (also configured via `fail_under=85` in `pyproject.toml`).
